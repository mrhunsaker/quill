...
| DOC-18 | Consolidate accessibility and engineering docs into the PRD; retire those folders | Docs | L | Todo | `docs/QUILL-PRD.md` is brought current with shipped behavior, then the full `docs/accessibility/` and `docs/engineering/` document sets are rolled up into the PRD as consolidated sections so the PRD is the single source of truth. Once their content lives in the PRD, the `docs/accessibility/` and `docs/engineering/` folders (and their generated `.html`/`.epub` artifacts) are retired, every cross-reference is updated, the docs-artifact gate is adjusted, and the PRD `.html`/`.epub` are regenerated via pandoc. A clean, polished, no-duplication documentation surface. |
