...
| A11Y-4 | Dialog escape and default audit, enforced by a contract guard | Accessibility | M | Todo | Every dialog has an Escape path and an explicit default, and focus returns to the editor on close. This is made un-regressable by a machine-enforced dialog-contract guard (a GATE-6-style conformance test) that fails when a `wx.Dialog` in the UI violates the construction contract that the find-in-files trap (#84) and the status-bar/misspelling layout bugs all shared: a `StdDialogButtonSizer` must be added with `wx.EXPAND` (never `wx.ALIGN_RIGHT`), the dialog must own an outer sizer wrapping its panel, an explicit default button and `apply_modal_ids` affirmative/escape id must be set, and the dialog must be `Destroy()`-ed. The guard steers new dialogs to the approved helpers (`quill/ui/dialog_contract.py`, the stock `wx.MessageDialog`/`SingleChoiceDialog`/`TextEntryDialog`, or the web `show_web_form`) so the bug class cannot recur. Pairs with the DLG cluster and GitHub issue #73. |
...
| OCR-1 | Native Windows OCR backend (zero-install) | Image to text | M | Todo | Add a `Windows.Media.Ocr` (WinRT) backend behind `core.ocr` so text can be pulled from images fully offline with no external install. `quill/io/ocr.py` becomes backend-pluggable (`windows`, `tesseract`) with a shared `OcrResult`; the Windows backend runs in the OCR worker process, returns text plus per-line confidence, and never touches the network. Core stays UI-agnostic; tests cover backend selection, the unavailable path on non-Windows, and result shaping. |
| OCR-2 | Tesseract opt-in backend via onboarding | Image to text | S | Todo | Keep the existing Tesseract path as a selectable backend the user turns on during a short OCR onboarding (engine choice auto/windows/tesseract, language pack note, install hint). A new `ocr_engine` setting drives selection; `auto` prefers the native Windows backend and falls back to Tesseract when present. When the chosen backend is unavailable the announced message points back to onboarding rather than failing silently. Tests cover engine resolution, the onboarding gate, and the fall-back order. |
| OCR-3 | Capture sources: file, clipboard, screen region | Image to text | M | Todo | OCR from an image file, a clipboard image, or a keyboard-driven screen-region capture, each bound under the QUILL key and the Tools menu, each announcing progress and outcome through the shared grammar (A11Y-1) and inserting text at the cursor. Fully keyboard operable with no mouse requirement; tests cover each capture path and the cancel path. |
| OCR-4 | Accessible OCR review and insert surface | Image to text | M | Todo | OCR output lands in a screen-reader-pageable review surface (engine, language, low-confidence note) before insertion, with insert, copy, and discard. Reuses the stock read-only control pattern; low-confidence lines are flagged for review. Gated by `core.ocr` (FLAG-1); tests cover rendering, the action keys, and the feature-off path. |
| OCR-5 | OCR as a Watch Profile action | Image to text | S | Todo | Register an "OCR to text" action in the WATCH-2 action registry so a watched folder of images becomes accessible text documents automatically, honoring `core.ocr` plus per-profile consent (WATCH-6) and the offline-by-default promise. Tests cover registration, the disabled-feature path, and a happy-path conversion. |
...
| MENU-1 | Unified Preferences home | Menus | M | In progress | Settings now open through one tabbed front door (`Ctrl+,` then `General`) built from the shared settings registry; remaining scattered Tools-menu toggles still need to fold in. |
| SET-1 | Per-feature settings groups | Settings | M | In progress | The Settings dialog renders one accessible `wx.Notebook` page per registry group (general, editing, navigation, accessibility, read aloud, AI, transcription, updates), each control generated from `quill/core/settings_registry.py` specs (bool, choice, int, float, text). Already-wired fields for side preview and CSV/Word open mode are now surfaced as registry specs. Remaining feature areas (snippets, macros, sticky notes, status bar) still need registry specs. |
| SET-2 | Tunable timing and pacing | Settings | M | In progress | QUILL key timeout, Quick Nav debounce and threshold, autosave interval, announcement throttle, Read Aloud rate, pitch, and sentence pause, and dictation sensitivity are now registry-backed `Settings` fields (clamped in `from_dict`) and auto-render in the tabbed dialog with per-setting reset. Read Aloud rate/pitch drive the engine live; the **autosave interval** field is consumed on startup and re-applied when the Settings dialog closes; the **announcement throttle** is now enforced in `_set_status`, which suppresses repeat status speech within the configured window. Remaining consumers (Quick Nav debounce/threshold) still need to read their new fields. |
| SET-3 | Tunable verbosity and announcements | Settings | M | In progress | A global `announcement_verbosity` level (minimal/normal/verbose) plus per-event overrides (`announce_wrap`, `announce_counts`, `announce_mode_changes`, `announce_spelling`, `announce_punctuation_level`) are registry-backed and surface in the Accessibility tab. Live consumers now wired: `announce_wrap` gates the " (wrapped)" suffix on Find/Replace; `announce_counts` decides whether the word-count summary is spoken or set quietly; `announce_mode_changes` gates QUILL key/browse mode enter/exit speech in `_quill_feedback` (sound still plays); `announce_spelling` short-circuits the passive live-misspelling hint. **Deferred (honest):** `announce_punctuation_level` is stored/surfaced but has no consumer yet — the Read Aloud engine takes no punctuation parameter — so this row stays In progress. |
| SET-4 | Tunable behavior toggles | Settings | M | In progress | Sticky browse, per-action confirmations, default export preset and new-document format, autoformat (smart quotes, dashes), and Quick Nav element inclusion (headings/links/lists) are registry-backed `Settings` fields and surface in the General/Editing/Navigation tabs; wrap navigation was already configurable. Live consumers now wired: `confirm_destructive_actions` gates the discard-changes prompt; `default_new_document_format` sets the Save-As file-type filter for new/pathless documents; `default_export_preset` preselects the Pandoc Conversion Wizard output format; `autoformat_smart_quotes`/`autoformat_dashes` drive inline typography in the editor CHAR_HOOK via the new wx-free `quill/core/autoformat.py`; `quick_nav_include_headings/links/lists` filter the Quick Nav index through `include_nav_items`. **Remaining (honest):** `browse_mode_sticky` is stored/surfaced but not yet consumed (the QK-5 second-press sticky lock is independent), so this row stays In progress. |
...
| SET-7 | Export, import, and pre-configure settings | Settings | S | In progress | The Settings dialog can export the full configuration to a `.qsf` file, import one back, and reset every setting to factory defaults (with a confirmation gate). Serialized through `registry.export_settings`/`import_settings` (schema_version 1). Admin pre-tune schema reference doc still pending (DOC-7). |
...
| WATCH-5 | Per-profile configuration and Settings group | Features | L | In progress | An accessible manager creates, edits, duplicates, enables, and deletes profiles. Each profile configures its folder, filters (suffix set, name patterns, min size and age, include-subfolders), action plus action options, schedule (always, interval, or quiet-hours window), and post-action handling (leave in place, move to a processed folder, or delete). A dedicated **Watch Folders** Settings group now holds the six global watch defaults (enable, default folder, include-subfolders, process-existing, auto-start, poll interval) and auto-renders in the tabbed dialog with per-setting reset, search, and export/import; the poll interval is clamped 2-300 s. Validation is live and errors are announced; the whole surface is keyboard and screen-reader navigable. Tests cover create/edit/delete, validation, and persistence. |
| WATCH-6 | Watch safety, consent, and resource governance | Features | M | Todo | Networked and AI watch actions stay opt-in per profile under the no-silent-network promise (AI-5): nothing reaches a cloud without explicit per-profile consent, and provider, model, and scope are shown. Wall-clock and concurrency caps (shared with the Python sandbox limits, SEC-9) prevent a runaway folder from overwhelming the machine; termination is announced. A dry-run preview shows exactly what a profile would do before it is armed. The atomic claim from WATCH-3 prevents double-processing. Tests cover the consent gate, the resource cap, dry-run, and the claim. |
| WATCH-7 | Built-in watch actions | Features | M | In progress | Ship the core action set on top of WATCH-2: Open, Convert or export to a chosen format (via the existing IO and Pandoc paths), Move/copy/archive to a destination, Run a named macro (FEAT-7), Run a saved Python transform (sandboxed, SEC-9), and consented AI actions (summarize, tag, rewrite to a style) via the assistant. Each action validates its options, announces its outcome, and routes failures to the queue's failed state with a clear reason. Tests cover each action's happy path and its failure path. |
...
| MENU-5 | User-customizable menus and context menu (Menu Editor) | Menus | L | In progress | A person can reorder, rename, and hide top-level menus, menu items, and editor context-menu entries, with one Reset to Factory Defaults, all through an accessible Menu Editor. The `wx`-free model layer landed in `quill/core/menu_customization.py` (delta-only, self-healing `reconcile`, versioned persistence) and covers the menu bar and the editor context menu through one shared `CONTEXT_MENU_KEY` model, with full unit coverage. The **top-level** Menu Editor now ships: Edit > Customize Menus... (`app.menu_editor`) opens a stock-control dialog (list + Move Up/Down, Rename, Show/Hide, Reset to Factory Defaults) that edits a working copy and persists only on Save, and a post-build transform pass applies the saved order/labels/visibility to the menu bar (bailing out untouched if any menu label is unrecognised). Remaining: the per-item and editor context-menu UI (the model already supports both). |
...
| --- | --- | --- |
| Tier 2 — Flagship | In progress | SET-1, SET-2, SET-3, SET-4, SET-7, WATCH-5, WATCH-7, MENU-1, MENU-5 |
| Tier 2 — Flagship | Todo | AGENT-1, AI-1, AI-6, AI-7, AI-14, AI-19, AI-20, AI-22, AI-24, OCR-1, OCR-2, OCR-3, OCR-4, OCR-5, SET-5, SHARE-1, SHARE-2, SHARE-3, CTX-1, DICT-2, FEAT-19, WATCH-6, FLAG-3, DLG-1 |
