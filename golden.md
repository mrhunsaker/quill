...
| DLG-1 | Migrate hand-rolled submit-once form dialogs to the shared accessible helper | Code quality | M | In progress | Executes the form wave of GitHub issue #73. The remaining hand-rolled multi-field `wx.Dialog` forms that are pure submit-once input (find-in-files / replace-in-files, Status Bar Layout, the read-aloud voice chooser, Watch Folder Settings, the YAML structure editor, and General Preferences) move onto the already-shipped `show_web_form` (`quill/ui/web_form.py`, accessible WebView with a native wx fallback) or, where a single field fits, a stock `wx.*` dialog. wxPython has no stock multi-field dialog, so this consolidates them on one tested, screen-reader-friendly surface rather than per-dialog focus/button code. Each migration keeps the existing inputs and outputs, lands behind a focused behavior test, and leaves the public surface unchanged (GATE-6). Until migrated, dialogs must still satisfy the A11Y-4 contract guard. **Delivered:** the Insert Link dialog (two-field display-text + URL submit-once form) is migrated off its hand-rolled `wx.Dialog` (HtmlWindow intro + two `wx.TextCtrl` + custom show/activate focus handlers) onto `show_web_form`, keeping identical inputs and outputs (prefilled selection, `https://` default, blank-URL cancel, markdown/HTML/plain link emission) behind three focused behavior tests; public surface unchanged. **In progress:** the named CRUD/conditional/reorder targets are poor submit-once fits and are deferred — find/replace-in-files carries a `DirPickerCtrl` Browse plus mode/output `wx.Choice`es, Watch Folder Settings is a list manager (Add/Edit/Duplicate/Toggle/Delete) whose per-profile editor has Browse buttons, conditional schedule/action fields, and a sandboxed Python code editor, the read-aloud chooser is a per-engine chain of stock choice/entry dialogs, Status Bar Layout has Move Up/Down reordering, and the YAML editor is a tree editor. These remain A11Y-4-compliant and are migrated only where a genuine submit-once shape exists, to avoid degrading working dialogs. |
...
| Tier 2 | Flagship experience | 58 | 46 | 12 | OCR-1, OCR-3, OCR-4, AGENT-1, AI-19, AI-24, SET-2, SET-3, CTX-1, DICT-2, FEAT-19, DLG-1 |
...
| --- | --- | --- |
| Tier 2 — Flagship | In progress | SET-2, SET-3, MENU-1, MENU-5, AGENT-1, OCR-1, OCR-3, OCR-4, CTX-1, DICT-2, FEAT-19, DLG-1, AI-24, AI-19 |
| Tier 2 — Flagship | Todo | (none) |
