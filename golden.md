...
| OCR-1 | Native Windows OCR backend (zero-install) | Image to text | M | In progress | `quill/io/ocr.py` is now backend-pluggable (`OcrBackend` Protocol, `WindowsOcrBackend`, `TesseractBackend`, shared `OcrResult`/`OcrLine` with per-line confidence) with `select_engine`/`available_engines` and a clean unavailable path; the WinRT backend lives in `quill/platform/windows/windows_ocr.py` (`Windows.Media.Ocr`, offline, no network). Tests cover backend selection, fall-back, the unavailable path, and Tesseract TSV per-line confidence parsing. **Remaining (honest, Linux/cloud blocker):** live WinRT recognition (`Windows.Media.Ocr`) is unverifiable in this environment (winsdk Python bindings not installed on Linux, WinRT APIs unavailable outside Windows). The `WindowsOcrBackend` engine surface and selection logic are wired and tested, but the actual WinRT `recognize` call cannot run here. **Windows finish-line:** Install QUILL on a real Windows machine, choose the Windows OCR engine in settings or let it auto-select, run OCR on a test image (PNG/JPG with known text content), and verify the recognized text is accurate and the confidence scores are sensible. Confirm the zero-install promise: no Tesseract download required when using the native backend. |
...
| OCR-3 | Capture sources: file, clipboard, screen region | Image to text | M | In progress | OCR from an image file is wired (`main_frame.ocr_image_file`, QUILL key + Tools menu, shared progress/outcome grammar, cancel path); the engine layer is capture-source-agnostic. **Remaining (honest, Linux/cloud blocker):** clipboard-image and keyboard-driven screen-region live capture require a real Windows display, clipboard access, and a working OCR engine, none of which are verifiable in this Linux environment. The file-based capture path is complete; clipboard and screen-region capture paths remain unimplemented. **Windows finish-line:** Implement `ocr_clipboard_image` (read image from clipboard via `win32clipboard` or similar, pass to `ocr_image`, insert result) and `ocr_screen_region` (overlay a keyboard-driven region selector on the screen, capture that region via screenshot API, pass to `ocr_image`, insert result). Wire both to Tools menu items and QUILL key bindings. Test on a real Windows machine with a working OCR backend, verify clipboard and screen-region capture succeed and insert recognized text into the editor. |
...
| AGENT-1 | Accessibility agent (make this document accessible) | AI | L | In progress | A consented, announced, reversible plan audits and fixes structure, alt text, link text, and plain language, then re-audits and reports; every step is a reviewable diff and a single undo. The deterministic engine landed in `quill/core/accessibility_agent.py` (wx-free, strict-typed) on top of the in-repo `glow.py` + `plain_language.py` primitives: `build_plan` audits structure (markdown heading spacing, heading-jump, html missing `lang`), alt text, link text, and plain language and returns an ordered plan of per-step-consented `AgentStep`s; `apply_plan` applies only the accepted auto-fixable steps and re-audits, returning a run report. The `AccessibilityAgentDialog` (Tools > AI > Make This Document Accessible..., command `tools.ai_accessibility_agent`) shows each step as an accessible `wx.CheckListBox` with a before/after details pane; auto-fixable steps are pre-checked, advisory steps are listed for review. Applying replaces the whole editor buffer as **one undo** and opens a re-audit report tab; the summary is announced. **Remaining (honest, by design):** structure and plain language are auto-fixed, but alt-text and link-text steps are **advisory-only** by design (flagged for human authoring, not auto-written by AI). This is not a cloud/Linux blocker — it is a deliberate product decision. AI-assisted alt-text and link-text authoring (calling an AI provider with image analysis or context to *suggest* alt text, then presenting it for user approval) is a future enhancement outside the scope of this row. **What "Done" means for AGENT-1:** The advisory behavior is intentional. This row can be marked Done when (1) the deterministic auto-fixes (structure, plain language) are verified on Windows to work correctly end-to-end, (2) the advisory alt-text/link-text steps are confirmed to flag those issues without auto-writing them, and (3) the dialog, consent, apply-as-one-undo, re-audit, and announcement flow are all verified accessible and correct on a real Windows installation. The absence of AI-assisted alt-text authoring does not block Done — that is a separate feature (call it AI-7b or AGENT-1b if it ships). |
